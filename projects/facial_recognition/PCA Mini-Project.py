...
# The F1 scores for Ariel Sharon for n_component values [10, 15, 25, 50, 100, 250] are [0.12,0.32,0.62,0.67,0.67,0.67]. The f1 score curve reaches a maxima with increase in number of principal components and then decreses gradually indicating that after a point (or maxima) , the f1 score decreases with increase in principal components. A higher f1 score indicates a good balance between accuracy and precision scores which does indicate that the classifier is doing well. But if your model should be better suited for precision or recall is a subjective call. 
